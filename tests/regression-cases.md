...
| Motor saturation | RCOU saturation finding raised. |
| Output mapping present | `SERVOx_FUNCTION` mapping limits motor-specific interpretation to mapped motor channels. |
| Known event window | `--window` outputs only use the requested TimeS range. |
| Derived plot | Custom plot expression such as `GPS.Alt-BARO.Alt` is generated without arbitrary Python evaluation. |
